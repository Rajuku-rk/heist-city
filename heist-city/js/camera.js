...
export function updateCamera(dt){ const a=actor(); if(!a) return;
  const aiming = S.firing && !S.fight.active && !S.driving;
  let targetYaw;
  ...
  const fovT = aiming?52:62; camera.fov += (fovT-camera.fov)*Math.min(1,8*dt); camera.updateProjectionMatrix();
