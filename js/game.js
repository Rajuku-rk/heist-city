...
import { buildCity, spawnAll, updateAlarm } from './world.js';
...
import { updateHeist } from './heist.js';

export function startGame(bot){ applyUpgrades(); applyScene(CFG.scenes[S.sceneIndex]); buildCity(); spawnAll(); initWeapons(); S.points=0; S.elapsed=0; S.mode='play'; S.bot=!!bot;
...
export function endGame(won){ if(S.mode!=='play') return; S.mode='over'; S.firing=false; addCredits(S.points); Audio.stop(); won?Audio.win():Audio.lose();
  const sc=el('endScreen'); sc.className='screen '+(won?'win':'lose'); el('endKicker').textContent=won?'Diamond Secured':'Caught';
